...
vertebrates <- arrange(vertebrates, id, year)

vertebrates2 <- vertebrates %>% group_by(id) %>%
  dplyr::slice(6:n())

vertebrates3 <- do.call(rbind, lapply(split(vertebrates, as.character(vertebrates$id)), function(x) x[-(1:5),]))



vertebrates <- vertebrates2  %>%  mutate(scalepop = (pop-min(pop))/(max(pop)-min(pop))) %>%
...
for(i in 1:nrow(vertebrates)){
  vertebrates$Years[i] <- sum(is.na(vertebrates[i, 26:69]) == FALSE)
}
...
# Population growth rate (R_t) based on predicted abundance ----
# after applying a Kalman filter

for(q in 1:length(Vert)){
  Vert[[q]]$R_t <- rep(NA, length = length(Vert[[q]]$Pred.N))
  for(z in 1:length(Vert[[q]]$Pred.N)){
    Vert[[q]]$R_t[z] <- log(Vert[[q]]$Pred.N[z + 1]/Vert[[q]]$Pred.N[z])}
}

# Create dataframe of all R_t over time for each population
Vert_abundance <- do.call("rbind", Vert)

# Save data frames ----
Vertebrate_data_left_trunc <- Vertebrate_data
write.csv(Vertebrate_data_left_trunc, "data/output/global_mus_left_trunc.csv")
#write.csv(Vert_abundance, "data/global_rates_scaled.csv")

ggplot() +
  geom_histogram(data = Vertebrate_data_left_trunc, aes(x = mu), 
                 fill = "white", alpha = 0.4, colour = "blue") +
  geom_histogram(data = mus, aes(x = mu), 
                 fill = "white", alpha= 0.4, colour = "green")
