...
load("D:/Gergana_Daskalova/gbif_1.RData")
...
# Comparing bird ranges with BirdLife ranges
bird_ranges <- read.csv("data/input/bird_ranges.csv")
colnames(bird_ranges) <- c("species", "person", "birdlife_range", "notes")
uk.mus.test <- inner_join(uk.mus, GBIF_ranges, by = "species")
uk.mus.test <- uk.mus.test %>% filter(Class == "Aves")

birdlife.comparison <- inner_join(bird_ranges, uk.mus.test, by = "species")
birdlife.comparison <- birdlife.comparison %>% drop_na(birdlife_range) %>%
  filter(birdlife_range != "EXTINCT")

birdlife.comparison$diff <- as.numeric(as.character(birdlife.comparison$birdlife_range)) - birdlife.comparison$range

(range.comp2 <- ggplot(birdlife.comparison, aes(x = as.numeric(as.character(birdlife_range)), 
                                            y = range)) +
    geom_point() +
    geom_smooth(method = "lm") +
    coord_equal() + 
